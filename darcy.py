...
if __name__ == '__main__':
    ...
    permeabilita_fratture = 0.001
    apertura_fratture = 0.01
    ...
    num_grids = len(gs)
    num_edges = len(ms)
    grid_dof = np.cumsum([g.num_cells for g in gs]); grid_dof = np.insert(grid_dof, 0, 0)
    edge_dof = np.cumsum([mg.num_cells for mg in mgs]); edge_dof = np.insert(edge_dof, 0, 0)
    ...
    exporter.write_vtu(['pressione']) # TODO: laambda non si puo' esportare? https://github.com/pmgbergen/porepy/issues/565
